# Google Certified Educator Level 2 - 工具導向考題與實作題手把手對照手冊
...
- **選擇題考點**：
  ...
  - 📝 **Q20 / Quiz B Q17 (Translate Document)**：點選「工具 $ightarrow$ 翻譯文件」產生多語言班級通訊。
- **對應 Lab 實作題步驟**：
...
- **選擇題考點**：
  ...
  - 📝 **Q10 / Quiz B Q7 (Appointment Schedule)**：親師面談點選「建立 $ightarrow$ 預約時間表」，生成公開 URL 讓家長點選預約。
...
- **選擇題考點**：
  ...
  - 📝 **Q16 / Quiz B Q9 (Co-teachers)**：跨校交流點選「成員 $ightarrow$ 新增教師」，將夥伴新增為協同教師 (`Co-teachers`)。
...
- **對應 Lab 實作題步驟**：
  ...
  - 🛠️ **Lab 1 Task 3**：進入「課堂作業」點選「建立 $ightarrow$ 主題」，分別建立 `Term 1` 與 `Term 2` 主題。
...
### 🎨 4. Google Slides (簡報)
- **選擇題考點**：
  - 📝 **Q18 / Quiz B Q8 (Link Media)**：點選「插入 $ightarrow$ 連結」或按右鍵選取「連結」加入外部資源。
  - 📝 **Q21 / Quiz B Q16 (Embed YouTube)**：點選「插入 $ightarrow$ 影片」直接在投影片內嵌 YouTube 教學影片。
  - 📝 **Q25 / Quiz B Q13 (Hyperlink Slides)**：物件按右鍵選「連結 $ightarrow$ 簡報中的投影片」，互相建立超連結製作單字記憶卡。
- **對應 Lab 實作題步驟**：
  ...
  - 🛠️ **Lab 3 Task 4**：點選「插入 $ightarrow$ 影片」內嵌影片檔案 `Meet Our Amazing New Teachers.mp4`。

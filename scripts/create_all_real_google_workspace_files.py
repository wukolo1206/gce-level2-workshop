...
print("All Google API services initialized!")

# Function to make file public
...
# Task 04 (Docs - Voice Feedback Mote)
...
# Task 05 (Docs - Translate Document)
...
# Task 13 (Slides - Flashcards Hyperlink)
...
# Task 14 (Slides - Theme Builder Placeholder)
...
# Task 15 (Slides - Video Embed & Assign)
...
# Task 18 (Sheets - Conditional Formatting)
...
# Task 19 (Sheets - Column Stats & Pivot Table)
...
# For remaining tasks, create Google Docs templates so ALL 25 tasks have real clickable links!
for t_num in range(6, 26):
    key = f"Task {t_num:02d}"
    if key not in real_links:
        t_doc = docs_service.documents().create(body={'title': f'{key} 研習真實線上實作檔'}).execute()
        t_id = t_doc.get('documentId')
        docs_service.documents().batchUpdate(documentId=t_id, body={'requests': [{'insertText': {'location': {'index': 1}, 'text': f"{key} 研習真實線上實作檔案\n\n請根據研習講義步驟，在此真實 Google Workspace 檔案中進行功能實操演練！\n"}}]}).execute()
        make_public(t_id)
        real_links[key] = {"title": f"{key} 研習真實線上實作檔", "type": "Docs", "url": f"https://docs.google.com/document/d/{t_id}/edit?usp=sharing"}

print(f"Created real files for ALL 25 TASKS!")

# Save to JSON
with open(r'd:\備課ai\研習講義\Google認證_Level2_研習講義與備考工具包\all_25_real_workspace_links.json', 'w', encoding='utf-8') as f:
    json.dump(real_links, f, ensure_ascii=False, indent=2)

print("Saved all 25 links to all_25_real_workspace_links.json!")
